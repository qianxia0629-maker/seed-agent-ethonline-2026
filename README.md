...
- The proof shows the first observed activity year, unique transactions in the last 90 days, active Uniswap V3 liquidity positions, and wallet-evidenced networks/protocols.
- The score is calculated from live indexed data, not generated by AI. Duplicate events in one transaction count once.
- The member card shows the score breakdown, recent verifiable records, The Graph source, and indexed block.
- A zero-evidence wallet receives `0`; missing evidence is never inferred or mocked.
- The score is explicitly not a credit score, proof of identity, or judgment of professional ability.

The complete formula and current coverage limits are documented in [`docs/ONCHAIN_PROOF_METHOD.md`](docs/ONCHAIN_PROOF_METHOD.md).
